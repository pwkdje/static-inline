...
void* rva2data(struct mach_header_64* header, uint64_t rva)
{
    ...
    if(header_vaddr != -1) {
        NSLog(@"hujiu-header_vaddr=%p", header_vaddr);
        rva += header_vaddr;
    }
...
NSMutableData* load_macho_data(NSString* path)
{
    ...
    if(magic==FAT_CIGAM)
    {
        ...
        NSLog(@"hujiu-add_hook_section nfat_arch=%d", NXSwapLong(fathdr->nfat_arch));
        if(NXSwapLong(fathdr->nfat_arch) != 1) {
            NSLog(@"hujiu-macho has too many arch!");
            return nil;
        }
        
        if(NXSwapLong(archdr->cputype) != CPU_TYPE_ARM64 || archdr->cpusubtype!=0) {
            NSLog(@"hujiu-macho arch not support!");
            return nil;
        }
        NSLog(@"hujiu-subarch=%x %x", NXSwapLong(archdr->offset), NXSwapLong(archdr->size));
    ...
    } else if(magic==FAT_CIGAM_64)
    {
        ...
        NSLog(@"hujiu-macho nfat_arch=%d", NXSwapLong(fathdr->nfat_arch));
        if(NXSwapLong(fathdr->nfat_arch) != 1) {
            NSLog(@"hujiu-macho has too many arch!");
            return nil;
        }
...
NSMutableData* add_hook_section(NSMutableData* macho)
{
    ...
    NSLog(@"hujiu-macho %x %x", header->magic, macho.length);
    ...
    struct load_command* lc = (struct load_command*)((UInt64)header + sizeof(*header));
    for (int i = 0; i < header->ncmds; i++) {
        NSLog(@"hujiu-macho load cmd=%d", lc->cmd);
        
        if (lc->cmd == LC_SEGMENT_64)
        {
            struct segment_command_64 * seg = (struct segment_command_64 *) lc;
            
            printf("hujiu-segment: %s file=%x:%x vm=%p:%p\n", seg->segname, seg->fileoff, seg->filesize, seg->vmaddr, seg->vmsize);
    ...
    if(!min_section_offset || !vm_end || !linkedit_seg) {
        NSLog(@"hujiu-cannot parse macho file!");
        return nil;
    }
    
    NSLog(@"hujiu-min_section_offset=%x vm_end=%p", min_section_offset, vm_end);
         ...
         i++, load_cmd = (struct load_command *)((uint64_t)load_cmd + load_cmd->cmdsize))
    {
      ...
      switch (load_cmd->cmd)
      {
          ...
          case LC_DYLD_INFO_ONLY:
          {
            ...
            NSLog(@"hujiu-[-] fix LC_DYLD_INFO_ done\n");
          } break;
          ...
          case LC_SYMTAB:
          {
            ...
            NSLog(@"hujiu-[-] fix LC_SYMTAB done\n");
          } break;
          ...
          case LC_DYSYMTAB:
          {
            ...
            NSLog(@"hujiu-[-] fix LC_DYSYMTAB done\n");
          } break;
          ...
          case LC_DYLD_CHAINED_FIXUPS:
          {
            ...
            NSLog(@"hujiu-[-] fix linkedit_data_command done\n");
          } break;
      }
    }
    
    if(min_section_offset < (sizeof(struct mach_header_64)+header->sizeofcmds)) {
        NSLog(@"hujiu-macho header has no enough space!");
        return nil;
    }
    ...
    if(chainedFixups)
   {
       NSLog(@"hujiu-chainedFixups %p %x", chainedFixups->dataoff, chainedFixups->datasize);
       ...
       NSLog(@"hujiu-chainsHeader offset=%x version=%d starts_offset=%x", offsetInLinkedit, chainsHeader->fixups_version, chainsHeader->starts_offset);
       ...
       NSLog(@"hujiu-startsInfo seg_count=%d", startsInfo->seg_count);
       ...
       *startsInfoNew = *startsInfo;
       ...
       for (uint32_t i=0; i < startsInfo->seg_count; ++i) {
           ...
           NSLog(@"hujiu-segInfoOffset[%d] %x", i, segInfoOffset);
           ...
           NSLog(@"hujiu-segInfo[%d] page_count=%d segment_offset=%p max_valid_pointer=%p",i, segInfo->page_count, segInfo->segment_offset, segInfo->max_valid_pointer);
...
NSString* StaticInlineHookPatch(char* machoPath, uint64_t vaddr, char* patch)
{
    ...
    // 初始化关键变量
    uint32_t cryptid = 0;                    // 加密标识，用于检查二进制文件是否加密
    struct mach_header_64* header = NULL;     // Mach-O文件头指针
    struct segment_command_64* text_seg = NULL; // Hook代码段指针
    struct segment_command_64* data_seg = NULL; // Hook数据段指针
    ...
    while(true) {
        ...
        for (int i = 0; i < header->ncmds; i++) {
            ...
            if(lc->cmd == LC_ENCRYPTION_INFO_64) {
                ...
                if(cryptid==0) cryptid = info->cryptid;  // 获取加密标识
            }
            // 移动到下一个加载命令
            lc = (struct load_command *) ((char *)lc + lc->cmdsize);
        }
        ...
        if(text_seg && data_seg) {
            NSLog(@"hujiu-hook section found!");
            break;
        }
        
        // 如果没找到Hook段，添加新的Hook段
        macho = add_hook_section(macho);
        if(!macho) {
            return @"hujiu-add_hook_section error!";
        }
    }
    
    if(cryptid != 0) {
        return @"hujiu-该app程序未砸壳!\nthis app is not decrypted!";
    }
    
    if(!text_seg || !data_seg) {
        return @"hujiu-无法解析machO文件!\ncan not parse machO file!";
    }
    ...
    if(patch && patch[0]) {
        ...
        // 打印调试信息
        NSLog(@"hujiu-codepath %p %s : %p~%p~%p %x", vaddr, patch, funcRVA, patch_end, code_end, patch_size);
        ...
        if(!hex2bytes(patch, (uint8_t*)patch_bytes+vaddr%4))
            return @"hujiu-修补字节码格式有误!\nThe bytes to patch are incorrect!";

    } else if(vaddr % 4) {
        // 如果没有补丁数据，检查地址是否4字节对齐
...
StaticInlineHookBlock* find_hook_block(void* base, uint64_t vaddr)
{
    ...
    if(!text_seg || !data_seg) {
        NSLog(@"hujiu-cannot parse hook info!");
        return NULL;
    }
    ...
    for(int i=0; i<STATIC_HOOK_CODEPAGE_SIZE/sizeof(StaticInlineHookBlock); i++)
    {
        ...
        {
            NSLog(@"hujiu-found hook block %d for %llX", i, vaddr);
            return &hookBlock[i];
        }
    }
...
void* StaticInlineHookFunction(char* machoPath, uint64_t vaddr, void* replace)
{
    ...
    if(!base) {
        NSLog(@"hujiu-cannot find module!");
        return NULL;
    }
    
    StaticInlineHookBlock* hookBlock = find_hook_block(base, vaddr);
    if(!hookBlock) {
        NSLog(@"hujiu-cannot find hook block!");
        return NULL;
    }
...
BOOL ActiveCodePatch(char* machoPath, uint64_t vaddr, char* patch)
{
    ...
    if(hookBlock->patch_hash != calc_patch_hash(vaddr, patch)) {
        NSLog(@"hujiu-code patch bytes changed!");
        return NO;
    }
